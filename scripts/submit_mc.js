...
function writeMinecraftData(uname ,cls_seat ,email ,mc_id) {
    ...
    .then( function(){ //doesn't work for some reason
        //success
        alert("success!");
        window.location = "../index.html";
    })
    .catch( function(error){ //doesn't work for some reason
        //error
        alert("failure!");
    });
}
...
function register_minecraft() {
    ...
    errmessage.appendChild(document.createTextNode("格式不正確."));
    var legal_form = true;

    if(!class_seat.match(/^[1-3][0-2][0-9][0-4][0-9]$/)){ //check if class_seat is valid
        legal_form = false;
        document.getElementById("class_seat_div").appendChild(errmessage);
    }
